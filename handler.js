 ...
 */
function Handler(config, blocked) {
    ...
    chrome.tabs.onActivated.addListener(
      function(activeInfo) {
        chrome.tabs.get(activeInfo.tabId, function(tab) {
          self._blocked.setCurrentFocus(tab.url);
        });
      }
    );
    chrome.windows.onFocusChanged.addListener(
      function(windowId) {
        if (windowId == chrome.windows.WINDOW_ID_NONE) {
          self._blocked.setCurrentFocus(null);
          return;
        }
        self._updateTimeWithCurrentTab();
      }
  ...
  Handler.prototype._updateTimeWithCurrentTab = function() {
    var self = this;
    chrome.tabs.query({active: true, lastFocusedWindow: true}, function(tabs) {
      if (tabs.length == 1) {
        ...
        chrome.windows.get(tabs[0].windowId, function(win) {
          ...
          self._blocked.setCurrentFocus(url);
        });
      }
    });
  };
